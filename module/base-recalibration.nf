...
/*
    ...
    input:
        ...
        split_interval: path to specific intervals file associated with input BAM
...
process run_ApplyBQSR_GATK {
    ...
    output:
    tuple val(sample_id), path("${output_filename}.bam"), path("${output_filename}.bam.bai"), val(interval_id), path(split_interval), val(has_unmapped), emit: output_ch_apply_bqsr
    tuple path(bam), path(bam_index), emit: output_ch_deletion
...
workflow recalibrate_base {
    ...
    */
    ...
    ided_recal_tables
        ...
        .map{ joined_input ->
            [
                ...
                joined_input[2].split_interval,
                joined_input[2].has_unmapped,
                joined_input[2].sample_id,
    ...
    run_ApplyBQSR_GATK.out.output_ch_apply_bqsr
        .map{ bqsred_sample ->
            [
                'sample_id': bqsred_sample[0],
                'bam': bqsred_sample[1],
                'bam_index': bqsred_sample[2],
                'interval_id': bqsred_sample[3],
                'split_interval': bqsred_sample[4],
